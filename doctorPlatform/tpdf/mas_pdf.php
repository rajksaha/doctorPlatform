...
function show_nextVisit($appointmentID,$xAxis,$yAxis,$size){
	...
	if($nextVisitType == 2){
		...
		$this->SetFont('prolog','',$size + 2);
		$this->MultiCell(60,5, "$newDate w`b ci Avevi Avm�eb|", 0);
	}
...
$pdf->Show_diagnosis($appointmentID,$rightXaxis,65,$size);

$rightYaxis = $pdf->Show_med($appointmentID,$rightXaxis,$rightYaxis,$size+3);
...
$leftYaxis=$pdf->Show_Past_History($appointmentID,$leftXaxis,$leftYaxis + 3, $maxX, $size, true);
